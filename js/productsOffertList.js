...
function renderProductsOffert(columData, columHtml){
    ...
    columData.forEach((p) => {
        ...
        let template = `
            <div class="item">
                <div class="media">
                    <div class="thumbnail object-cover">
                        <a href="view/detailView.php?index=${p.id_prod}">
                            <img src="${decode_img(p.img_producto_frente)}" alt="">
                        </a>
                    </div>
                    <div class="hoverable">
                        <ul>
                ...
                <div class="content">
                    ...
                    <div class="rating">
                        <div class="stars"></div>
                        <span class="mini-text">${'(' + p.producto_comentario.split(";").length + ') Valoraciones'}</span>
                    </div>
                    <div class="price">
                        <span class="current">${USDollar.format(p.precio_producto - ((p.oferta_producto * p.precio_producto)) /100)}</span>
                        <span class="normal mini-text">${ USDollar.format(p.precio_producto)}</span>
                    </div>
                    <div class="mini-text">
        ...
        columHtml.innerHTML += template;
        
    })

}
renderProductsOffert(colum1Offert, columnLeft);
renderProductsOffert(colum2Offert, columnRight);
